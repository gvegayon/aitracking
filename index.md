...
Functions look for a GitHub token via
[`gh_token()`](https://gvegayon.github.io/aitracking/reference/gh_token.md):
an explicit `token` argument, the `GITHUB_PAT`/`GITHUB_TOKEN`
environment variables, or the git credential store (if the `gitcreds`
package is installed). Unauthenticated requests work but are
rate-limited to 60 calls/hour (5,000/hour with a token).
...
|----|----|
| [`gh_commits()`](https://gvegayon.github.io/aitracking/reference/gh_commits.md) | Commit history (author, hash, message, time) of one or more repos |
| [`gh_commit_lines()`](https://gvegayon.github.io/aitracking/reference/gh_commit_lines.md) | Lines added/deleted per commit (optionally per file) |
| [`gh_interactions()`](https://gvegayon.github.io/aitracking/reference/gh_interactions.md) | Issues, PRs, and comments–including human requests to AI agents |
| [`gh_assignments()`](https://gvegayon.github.io/aitracking/reference/gh_assignments.md) | Issues/PRs and who they are assigned to, flagging AI agents |
| [`gh_pulls()`](https://gvegayon.github.io/aitracking/reference/gh_pulls.md) | Pull requests, including the head branch (agent branch prefixes) |
| [`gh_traffic()`](https://gvegayon.github.io/aitracking/reference/gh_traffic.md) | Clone/view history (last 14 days, requires push access) |
| [`gh_downloads()`](https://gvegayon.github.io/aitracking/reference/gh_downloads.md) | All-time download counts of release assets |
| [`gh_languages()`](https://gvegayon.github.io/aitracking/reference/gh_languages.md) | Current language composition of a repo |
| [`ai_classify()`](https://gvegayon.github.io/aitracking/reference/ai_classify.md) | Tag rows where an AI agent was involved (or mentioned) |
| [`loc_evolution()`](https://gvegayon.github.io/aitracking/reference/loc_evolution.md) | Evolution of project size (net lines of code, by language) |
| [`plot_timeline()`](https://gvegayon.github.io/aitracking/reference/plot_timeline.md) | Activity timeline by commits or by lines added/deleted |
| [`gh_copilot_metrics()`](https://gvegayon.github.io/aitracking/reference/gh_copilot_metrics.md) / [`gh_copilot_seats()`](https://gvegayon.github.io/aitracking/reference/gh_copilot_seats.md) | Org-level Copilot usage (see below) |
| [`gh_api()`](https://gvegayon.github.io/aitracking/reference/gh_api.md) / [`gh_token()`](https://gvegayon.github.io/aitracking/reference/gh_token.md) | Low-level API access and token discovery |
...
combines the traces that projects and agents actually leave, and reports
them in two tiers:
...
wraps GitHub’s Copilot usage metrics API (suggestion/acceptance counts,
lines suggested and accepted, active users, chat activity). Two caveats
matter:
...
is far more accessible (it needs only `read:org`) and reports per-user
seat assignment and last activity.
